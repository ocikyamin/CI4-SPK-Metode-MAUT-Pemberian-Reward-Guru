    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <form id="forms-user" method="post">
                ...
                <input type="hidden" name="akses_id" value="<?=$akses['id']?>">
                <input type="hidden" name="old_email" value="<?=$user['email']?>">
                <div class="modal-body">
                    <div class="form-group mb-2">
                        <label for="fullname">Full Name</label>
                        <input type="text" class="form-control" id="fullname" name="fullname"
                            placeholder="Ex : Abdul Yamin" value="<?=$user['full_name']?>">
                        <div class="fullname invalid-feedback"></div>
                    </div>
                    <div class="form-group mb-2">
                        <label for="email">Email</label>
                        <input type="text" class="form-control" id="email" name="email"
                            placeholder="Ex : ocikyamin@gmail.com" value="<?=$user['email']?>">
                    ...
                    <div class="role-akses form-check form-radio-success mb-2">
                        <input type="radio" id="role-<?=$d['id']?>" name="role" value="<?=$d['id']?>"
                            class="role form-check-input" <?=$d['id']==$akses['role_user_id'] ? 'checked':null?>>
                        <label class="form-check-label" for="role-<?=$d['id']?>"><?=$d['role_name']?></label>
                    </div>
                    <?php } ?>
                    <div class="txtrole invalid-feedback"></div>
                    <div id="supervisor" class="form-group mb-2  <?=$akses['sekolah_id']!== NULL  ? '':'d-none'?>">
                        <label for="sekolah">Supervisor Untuk Sekolah ?</label>
                        <select name="sekolah" id="sekolah" class="form-control">
                            <option value="">Pilih Sekolah</option>
                            <?php
                    foreach (Tabel('sekolah') as $d) {?>
                            <option value="<?=$d['id']?>" <?=$d['id']==$akses['sekolah_id'] ? 'selected':null ?>>
                                <?=$d['nama_sekolah']?></option>
                            <?php } ?>
                        </select>
                    </div>
