<div id="modal-guru-mapel" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="standard-modalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="form-group row mb-2 mt-2">
            <label class="col-lg-3" for="sekolah">Periode</label>
            <div class="col-lg-9">
            ...
            <div class="col-lg-9">
            <input type="text" class="form-control form-control-sm" name="kode_guru" id="kode_guru" placeholder="Contoh : A1">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" id="btn-save" class="btn btn-success btn-rounded shadow-sm"><i class="mdi mdi-send-check"></i> Simpan Pengaturan</button>
            </div>
            </form> <!-- Form //-->
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
...
$(document).ready(function () {
    ...
    ListKelas(sekolah_id);
});

function ListKelas(id) {
    $.ajax({
        url: "<?=base_url('admin/guru/mapel/kelas')?>",
        data: {id:id},
        dataType: "json",
        success: function (response) {
            $('#kelas').html('<option value="">Pilih Kelas</option>'+response.list_kelas)
        }
    });
}
 ...
 $('#form-guru-mapel').submit(function (e) { 
    ...
    $.ajax({
        type: "post",
        url: "<?=base_url('admin/guru/mapel/save')?>",
        data: $(this).serialize(),
        dataType: "json",
                    beforeSend: function() {
                    ...
                    $('#btn-save').html(`<i class="mdi mdi-send-check"></i> Simpan Pengaturan`);
                    },
        success: function (response) {
            if (response.error) {
